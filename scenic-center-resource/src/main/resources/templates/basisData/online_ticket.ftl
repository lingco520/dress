<!DOCTYPE html>
<html>
<head>
<#include "../header.ftl">
<#include "../edit.ftl">
    <link rel="stylesheet" href="/styleSheet/daqStyle.css">
    <script type="text/javascript" src="/script/jQuery/jquery.lSelect.js"></script>
    <style>
        /*时间验证错误提示，没有使用form-item-table 特殊处理*/
        .dateError {
            float: left;
            display: inline-block;
            line-height: 36px;
            margin-left: 10px;
            padding-left: 20px;
            font-size: 12px;
            color: #ff4949;
            background: url(data:image/png;base64,) no-repeat 0 center
        }
    </style>
</head>
<body>
<div class="system-top">
    <p class="daq-crumb clearfix"><a href="javascript:;" class=""><i class="sysfont"></i> 数据模拟</a>
        <i class="sysfont crumb-sign sysfont"></i><a href="javascript:;" class=""><i class="sysfont"></i>
            线上票务</a>
    </p>
</div>
<div class="section-content">
    <div class="section-son-content">
        <div class="form-content">
            <div class="base-data-tit ml-10">
                <h3>线上票务</h3>
            </div>
            <form id="dataConfigForm" action="">
                <input type="hidden" name="id" value="${id!}"/>
                <div class="form-item-table">
                    <div class="tab-td-wid100">
                        <table cellpadding="0" cellspacing="0" width="100%">
                            <!--数据模拟时间选择-->
                            <tr>
                                <td class="tab-label-color"><label class="form-label">开始时间:</label></td>
                                <td>
                                    <div class="ml-10 mr-10">
                                        <p class="daq-inp-rule daq-inp-date wid-139">
                                            <input class="date" name="dontStartDate" id="dontStartDate"
                                                   placeholder="请选择开始时间"/>
                                            <i class="sysfont"></i>
                                        </p>
                                    </div>
                                </td>
                                <td class="tab-label-color"><label class="form-label">结束时间:</label></td>
                                <td>
                                    <div class="ml-10 mr-10">
                                        <p class="daq-inp-rule daq-inp-date wid-139">
                                            <input class="date" name="dontEndDate" id="dontEndDate"
                                                   placeholder="请选择开始时间"/>
                                            <i class="sysfont"></i>
                                        </p>
                                    </div>
                                </td>
                                <td class="tab-label-color"><label class="form-label">是否覆盖原有数据:</label></td>
                                <td>
                                    <div class="form-table-edit">
                                        <div id="checkbox" class="mt-20 clearfix choose-box ">
                                            <input type="checkbox" name="isDeleteHistory" value="1" title="是"/>
                                        </div>
                                    </div>
                                </td>
                                <td colspan="3"></td>
                            </tr>
                    <#if dtoList??>
                        <#list dtoList as dto>
                        <tr class="new">
                            <td class="tab-label-color"><label class="form-label">票名称:</label></td>
                            <td>
                                <div class="form-table-edit">
                                    <div class="daq-inp-rule">
                                        <input type="text" name="dontName" class="s_input s_input_fb w318"
                                               validate="{maxlength:40}" value="${dto.dontName!}"/>
                                    </div>
                                </div>
                            </td>
                            <td class="tab-label-color"><label class="form-label">票单价:</label></td>
                            <td>
                                <div class="form-table-edit">
                                    <div class="daq-inp-rule">
                                        <input type="text" name="dontPrice" class="s_input s_input_fb w318"
                                               validate="{digits: true,range:[0,99999]}" value="${dto.dontPrice!}"/>
                                    </div>
                                </div>
                            </td>
                            <td class="tab-label-color"><label class="form-label">票数量:</label></td>
                            <td>
                                <div class="form-table-edit">
                                    <div class="daq-inp-rule">
                                        <input type="text" name="dontNumber" class="s_input s_input_fb w318"
                                               validate="{digits: true,range:[1,100]}" value="${dto.dontNumber!}"/>
                                    </div>
                                </div>
                            </td>
                            <td class="tab-label-color"><label class="form-label">记录数范围:</label></td>
                            <td>
                                <div class="form-table-edit wid-78 fl">
                                    <div class="daq-inp-rule">
                                        <input type="text" name="dontRandomMin" class="s_input s_input_fb"
                                               validate="{digits: true,range:[1,99999],messages:{digits: '请输入非负整数'}}"
                                               value="${dto.dontRandomMin!}"/>
                                    </div>
                                </div>
                                <div class="form-table-edit wid-78 fl">
                                    <div class="daq-inp-rule">
                                        <input type="text" name="dontRandomMax" class="s_input s_input_fb"
                                               validate="{digits: true,range:[1,99999],messages:{digits: '请输入非负整数'}}"
                                               value="${dto.dontRandomMax!}"/>
                                    </div>
                                </div>
                            </td>
                            <td class="tab-label-color">
                                <div class="demo-button ml-10">
                                    <div class="daq-btn-line btn-icon btn-hover add"><i class="sysfont"></i></div>
                                    <div class="daq-btn-line btn-icon btn-hover1 remove"><i class="sysfont"></i></div>
                                </div>
                            </td>
                        </tr>
                        </#list>
                    </#if>
                            <!--线上票务结束-->
                            <tr>
                                <td class="dq_dt_ltd" colspan="10">
                                    <div class="form-table-edit">
                                        <input type="button" class="daq-btn btn-pad-30 media-btn mt-5 nextorlast"
                                               value="生成模拟数据"/>
                                    </div>
                                </td>
                            </tr>
                        </table>
                    </div>
                </div>
            </form>
        </div>
    </div>
</div>
<div class="loading"></div>
</body>
<script>
    $(function () {
        // 重写validate
        if ($.validator) {
            $.validator.prototype.elements = function () {
                var validator = this,
                        rulesCache = {};
                // select all valid inputs inside the form (no submit or reset buttons)
                return $(this.currentForm)
                        .find("input, select, textarea")
                        .not(":submit, :reset, :image, [disabled]")
                        .not(this.settings.ignore)
                        .filter(function () {
                            if (!this.name && validator.settings.debug && window.console) {
                                console.error("%o has no name assigned", this);
                            }
                            rulesCache[this.name] = true;
                            return true;
                        });
            }
        }
        /* ---------- 表单验证 ---------- */
        var $validateForm = $("#dataConfigForm");
        var formSubmitted = false;
        if ($validateForm.length > 0) {
            $.metadata.setType("attr", "validate");
            $validateForm.validate({
                //当验证成功时,调用此函数.
                submitHandler: function () {
                    console.log("validate successfully");
                    saveForm($validateForm);
                    /*//如果表单验证成功,屏蔽掉提交按钮防止重复提交
                    if (!formSubmitted) {
                        formSubmitted = true;
                        saveForm($validateForm);
                    } else {
                        layer.msg("请勿重复提交!");
                    }*/
                }, errorPlacement: function (error, element) {
                    error.appendTo(element.parents(".form-table-edit"));
                    if(element.hasClass("date")){
                        // 时间错误信息追加
                        error.appendTo(element.parents(".mr-10"));
                        error.addClass("dateError")
                    }
                }
            });
        }
        $(".nextorlast").click(function () {
            $("#dataConfigForm").submit();
        });
        // 声明遮罩层
        var load;

        function saveForm(validateForm) {
            var param = validateForm.serialize();
            var _csrf = document.cookie.split("=")[1];
            $.ajax({
                headers: {'X-XSRF-TOKEN': _csrf},
                url: "/permitted/basisData/saveDataOnlineTicket",
                type: "post",
                data: param,
                dataType: "json",
                beforeSend: function () {
                    // 考虑到原遮罩层 hide() 后不能再次生效，此处先克隆一个原始对象，每次hide克隆后的对象
                    $(".loading").after($(".loading").clone(true));
                    // 添加一个hide时的类属性，标记此div需要隐藏
                    $(".loading:last").addClass("daq-loading")
                    // 发送请求前添加遮罩层
                    load = $(".daq-loading").daqBlockLoading({
                        position: "absolute"
                    });
                    $(".daq-loading").find(".load-text").html("数据写入中，请稍等");
                },
                success: function (result) {
                    // 成功后，关闭遮罩层
                    load.hide();
                    layer.alert(result.message);
                }
            });
        }

        // 自定义验证表达式
        $.validator.addMethod("checkDontRandomMin", function (value, element, params) {
            var dontRandomMax = $(element).closest('.new').find("[name=dontRandomMax]").val();
            if (dontRandomMax !== null && dontRandomMax !== "" && dontRandomMax !== undefined) {
                return this.optional(element) || (parseInt(dontRandomMax) >= parseInt(value));
            }
            return true;
        }, "小于最大值");
        $.validator.addMethod("checkDontStartDate", function (value, element, params) {
            var dontEndDate = $("#dontEndDate").val();
            if (dontEndDate !== null && dontEndDate !== "" && dontEndDate !== undefined) {
                return this.optional(element) || (dontEndDate >= value);
            }
            return true;
        }, "不大于结束日期");

        // 绑定验证
        $("#dontStartDate").rules("add", {
            required: true,
            checkDontStartDate: true,
            messages: {
                required: "必填"
            }
        });
        $("#dontEndDate").rules("add", {
            required: true,
            messages: {
                required: "必填"
            }
        });
        $("[name=dontName]").each(function () {
            $(this).rules("add", {
                required: true,
                messages: {
                    required: "必填"
                }
            });
        });
        $("[name=dontPrice]").each(function () {
            $(this).rules("add", {
                required: true,
                messages: {
                    required: "必填"
                }
            });
        });
        $("[name=dontNumber]").each(function () {
            $(this).rules("add", {
                required: true,
                messages: {
                    required: "必填"
                }
            });
        });
        $("[name=dontRandomMin]").each(function () {
            $(this).rules("add", {
                required: true,
                checkDontRandomMin: true,
                messages: {
                    required: "必填"
                }
            });
        });
        $("[name=dontRandomMax]").each(function () {
            $(this).rules("add", {
                required: true,
                messages: {
                    required: "必填"
                }
            });
        });
    })
</script>
<script type="text/javascript">
    $(function () {
        // 数据时间段插件js
        var start = laydate.render({
            elem: '#dontStartDate',
            type: 'date',
            value: '${(dontStartDate?string('yyyy-MM-dd'))!}',
            change: function () {

            },
            done: function (value, date) {
                if (value !== '') {
                    end.config.min.year = date.year;
                    end.config.min.month = date.month - 1;
                    end.config.min.date = date.date;
                } else {
                    end.config.min.year = '';
                    end.config.min.month = '';
                    end.config.min.date = '';
                }
            }
        });
        var end = laydate.render({
            elem: '#dontEndDate',
            type: 'date',
            value: '${(dontEndDate?string('yyyy-MM-dd'))!}',
            change: function () {

            },
            done: function (value, date) {
                if (value !== '') {
                    start.config.max.year = date.year;
                    start.config.max.month = date.month - 1;
                    start.config.max.date = date.date;
                } else {
                    start.config.max.year = '';
                    start.config.max.month = '';
                    start.config.max.date = '';
                }
            }
        });

        // 复选框绑定
        $("#checkbox").daqCheckbox({
            callback: function (data, obj) {
            }
        });

        var newTr = '<tr class="new">' +
                '<td class="tab-label-color"><label class="form-label">票名称:</label></td>' +
                '<td>' +
                '<div class="form-table-edit">' +
                '<div class="daq-inp-rule">' +
                '<input type="text" name="dontName" class="s_input s_input_fb w318" validate="{maxlength:40}" value="">' +
                '</div></div></td>' +
                '<td class="tab-label-color"><label class="form-label">票单价:</label></td>' +
                '<td><div class="form-table-edit">' +
                '<div class="daq-inp-rule">' +
                '<input type="text" name="dontPrice" class="s_input s_input_fb w318" validate="{digits: true,range:[0,99999]}" value="">' +
                '</div></div></td>' +
                '<td class="tab-label-color"><label class="form-label">票数量:</label></td>' +
                '<td> <div class="form-table-edit"><div class="daq-inp-rule">' +
                '<input type="text" name="dontNumber" class="s_input s_input_fb w318" validate="{digits: true,range:[1,100]}" value="">' +
                '</div></div></td>' +
                '<td class="tab-label-color"><label class="form-label">记录数范围:</label></td>' +
                '<td><div class="form-table-edit wid-78 fl">' +
                '<div class="daq-inp-rule">' +
                '<input type="text" name="dontRandomMin" class="s_input s_input_fb" validate="{digits: true,range:[1,99999],messages:{digits: \'请输入非负整数\'}}" value="">' +
                '</div></div><div class="form-table-edit wid-78 fl">' +
                '<div class="daq-inp-rule">' +
                '<input type="text" name="dontRandomMax" class="s_input s_input_fb" validate="{digits: true,range:[1,99999],messages:{digits: \'请输入非负整数\'}}" value="">' +
                '</div></div></td>' +
                '<td class="tab-label-color">' +
                '<div class="demo-button ml-10">' +
                '<div class="daq-btn-line btn-icon btn-hover add"><i class="sysfont"></i></div> ' +
                ' <div class="daq-btn-line btn-icon btn-hover1 remove"><i class="sysfont"></i></div>' +
                '</div></td></tr>';

        // copy DOM
        $(document).on("click", '.add', function () {
            // 限制个数
            var trs = $("table tr[class=new]").length;
            if (trs >= 10) {
                layer.alert("最多10个节点");
                return;
            }
            $(".new:last").after(newTr);

            var idstr = 1;
            $("[name=dontName]").each(function () {
                $(this).attr("id", "dontName" + idstr);
                idstr++;
            });
            $("[name=dontPrice]").each(function () {
                $(this).attr("id", "dontPrice" + idstr);
                idstr++;
            });
            $("[name=dontNumber]").each(function () {
                $(this).attr("id", "dontNumber" + idstr);
                idstr++;
            });
            $("[name=dontRandomMin]").each(function () {
                $(this).attr("id", "dontRandomMin" + idstr);
                idstr++;
            });
            $("[name=dontRandomMax]").each(function () {
                $(this).attr("id", "dontRandomMax" + idstr);
                idstr++;
            });
        })

        $(document).on("click", '.remove', function () {
            // 判断是否是最后一个
            var trs = $("table tr[class=new]").length;
            if (trs <= 1) {
                layer.alert("已经是最后一个节点，不能删除");
                return;
            }
            $(this).closest('.new').remove();
        });
    })


</script>
</html>