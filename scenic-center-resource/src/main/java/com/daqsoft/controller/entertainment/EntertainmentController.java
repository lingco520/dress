package com.daqsoft.controller.entertainment;
...
import com.daqsoft.service.entertainment.EntertainmentService;
...
public class EntertainmentController extends BaseResponseController {

    private static final Logger LOGGER = LoggerFactory.getLogger(EntertainmentController.class);
    private static final String PATH = "entertainment/";
    @Autowired
    ...
    @Autowired
    public EntertainmentController(SysRegionServiceImpl sysRegionService) {
    ...
    public String list(ModelMap model) {
        //获取餐饮等级
        List<SysDictVo> entertainmentTypeDict = commonService.getDictByType(DictConstants.ENTERTAINMENT_TYPE);
        model.addAttribute("entertainmentType", entertainmentTypeDict);
        return PATH + "entertainment_list";
    }
    ...
    @ResponseBody
    public BaseResponse list(PageForm pageForm, String name,String type,String status, HttpServletRequest request) {
        try {
            return pageBuilderSuccess(entertainmentService.list(pageForm, name, getVcode(request),type,status));
        } catch (Exception e) {
            LOGGER.error("查询错误:", e);
    ...
    public String edit(Long id, ModelMap model, HttpServletRequest request) throws Exception {
        ResEntertainment dto = new ResEntertainment();
        //获取餐饮等级
        List<SysDictVo> entertainmentTypeDict = commonService.getDictByType(DictConstants.ENTERTAINMENT_TYPE);
        try {
            if (id != null) {
                dto = entertainmentService.get(id);
            } else {
                dto = new ResEntertainment();
            }
        } catch (Exception e) {
        ...
        model.addAttribute("entertainmentType", entertainmentTypeDict);
        return PATH + "entertainment_input";
    }
    ...
    @ResponseBody
    public BaseResponse save(@Valid @ModelAttribute(value = "dto") ResEntertainment dto, HttpServletRequest request) {
        dto.setVcode(getVcode(request));//设置vcode
        try {
            //当前登录用户
            if (dto.getId() == null) {
                entertainmentService.save(dto);
            } else {
                entertainmentService.update(dto);
            }
        } catch (Exception e) {
            LOGGER.error("保存失败:", e);
