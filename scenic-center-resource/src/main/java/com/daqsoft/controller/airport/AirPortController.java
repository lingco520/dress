package com.daqsoft.controller.airport;
...
public class AirPortController extends BaseResponseController {
    ...
    public BaseResponse list(PageForm pageForm, String name, String address, HttpServletRequest request) {
        try {
            ...
            return pageBuilderSuccess(airPortService.list(pageForm, name, address));
        } catch (Exception e){
            LOGGER.error("查询错误:", e);
    ...
    @ResponseBody
    public BaseResponse save(@Valid @ModelAttribute(value = "dto") AirPort dto, HttpServletRequest request) {
        try {
            dto.setVcode(getVcode(request));
            dto.setUpdateTime(new Date());
            //当前登录用户
            if (dto.getId() == null) {
                airPortService.save(dto);
            } else {
                airPortService.update(dto);
            }
        } catch (Exception e) {
            LOGGER.error("保存失败:", e);
