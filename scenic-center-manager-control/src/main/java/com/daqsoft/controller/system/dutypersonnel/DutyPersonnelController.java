package com.daqsoft.controller.system.dutypersonnel;
...
import com.daqsoft.service.dutypersonnel.DutyPersonnelService;
 ...
 */
@Controller
@RequestMapping(value = "/dutyPersonnel")
public class DutyPersonnelController extends BaseResponseController {

    private static final Logger LOGGER = LoggerFactory.getLogger(DutyPersonnelController.class);
    private static final String PATH = "dutyPersonnel/";
    @Autowired
    private DutyPersonnelService dutyPersonnelService;
    @Autowired
    ...
    public String list(ModelMap model) {
        return PATH + "dutyPersonnel_list";
    }
    ...
    public BaseResponse list(PageForm pageForm, String name,String status, HttpServletRequest request) {
        try {
            return pageBuilderSuccess(dutyPersonnelService.list(pageForm, name, getVcode(request),status));
        } catch (Exception e) {
            LOGGER.error("查询错误:", e);
    ...
    public String edit(Long id, ModelMap model, HttpServletRequest request) throws Exception {
        SysDutypersonnel dto = new SysDutypersonnel();
        // 性别
        List<SysDictVo> gender = commonService.getDictByType(DictConstants.GENDER);
        // 部门
        List<SysDictVo> department = commonService.getDepartment(getVcode(request));
        // 账号
        List<SysDictVo> manager = commonService.getAccount(getVcode(request));
        // 职位
        List<SysDictVo> duty = commonService.getDictByType(DictConstants.DUTY_TYPE);
        // 学历
        List<SysDictVo> education = commonService.getDictByType(DictConstants.EDUCATION);
        try {
            if (id != null) {
                dto = dutyPersonnelService.get(id);
            } else {
                dto = new SysDutypersonnel();
            }
        } catch (Exception e) {
        ...
        return PATH + "dutyPersonnel_input";
    }
    ...
    @ResponseBody
    public BaseResponse save(@Valid @ModelAttribute(value = "dto") SysDutypersonnel dto, HttpServletRequest request) {
        dto.setVcode(getVcode(request));//设置vcode
        try {
            //当前登录用户
            if (dto.getId() == null) {
                dutyPersonnelService.save(dto);
            } else {
                dutyPersonnelService.update(dto);
            }
        } catch (Exception e) {
            LOGGER.error("保存失败:", e);
    ...
    public BaseResponse delete(Long[] ids) {
        try {
            dutyPersonnelService.delete(ids);
        } catch (Exception e) {
            LOGGER.error("删除失败", e);
